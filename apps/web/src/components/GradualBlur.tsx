...
const useResponsiveDimension = (
	...
	useEffect(() => {
		...
		const calc = () => {
			...
			const k = cap(key as string) as keyof GradualBlurProps;
			...
			if (w <= 480 && config[mobileKey]) v = config[mobileKey] as string | undefined;
			else if (w <= 768 && config[tabletKey]) v = config[tabletKey] as string | undefined;
			else if (w <= 1024 && config[desktopKey]) v = config[desktopKey] as string | undefined;
			setVal(v);
		};
